...
const ShowDetails = () => {
    ...
    useEffect(() => {
        const isInt = /^[0-9]+$/;
        if (param.id.match(isInt)) {
            axios.get(`https://api.tvmaze.com/shows/${param.id}`)
                .then(res => {
    ...
    const ErrorPage = () => {
        return (
            <div>
                ...
                <div className="d-flex justify-content-center center" style={{ textAlign: 'center' }}>
                    <div>
                        <h1>
                            505
                </h1>
    ...
    return (
       <div>
            <div className="container productpage" >
                <div className="bg_img" style={showInfo.image ? {backgroundImage: "url("+showInfo.image.original+")"} : {backgroundImage: "" }}></div>
            { !isError ? showInfo ?
                <div>
                    <div className="row product_box">
                        ...
                        <div className="img_col col-lg-4 col-md-4">
                            {showInfo.image ? <img className="productimg" src={showInfo.image.original} alt={showInfo.name} /> : <img src="https://tileofspainusa.com/wp-content/plugins/ultimate-product-catalogue/images/No-Photo-Available.png" height="270px" alt="no-img-found" />}
                        </div>
                        <div className="col-lg-8 col-md-8 white_color">
                            ...
                            <p className="desc" dangerouslySetInnerHTML={{ __html: showInfo.summary }}></p>
                            {
                                showInfo.rating.average ?
                                    <div className="rating_box">
                                        Avarage user rating
                                <div>
                                            <span className="ratecount">{showInfo.rating.average}</span> / 10
                                </div>
                                    </div> : ""
                            }
                            {/* {
                        showInfo.externals ? 
                            <div className="rating_box">
                                External Links
                                <div>
                                    <a href={`https ://www.imdb.com/title/${showInfo.externals.imdb}`} target="_blank" >
